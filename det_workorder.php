...
include 'koneksi.php';
$idwo = $_GET['id'];
$coba22 = sqlsrv_query($koneksi, "SELECT * FROM [apd_base].[dbo].[work_order],[apd_base].[dbo].[gi],[apd_base].[dbo].[pen],[apd_base].[dbo].[bagian],[apd_base].[dbo].[user] WHERE work_order.id_gi=gi.id_gi AND work_order.id_pen=pen.id_pen AND work_order.id_bag=bagian.id_bag AND work_order.id_user=[apd_base].[dbo].[user].id_user AND id_wo='$idwo'");
$cb2 = sqlsrv_query($koneksi, "SELECT * FROM [gi]");

$coba23 = sqlsrv_query($koneksi, "SELECT *,[drop_wo].[keterangan] as ketu FROM [apd_base].[dbo].[drop_wo],[work_order],[bagian],[user] WHERE drop_wo.id_wo=work_order.id_wo AND drop_wo.id_bag=bagian.id_bag AND drop_wo.id_user=[dbo].[user].id_user  AND drop_wo.id_wo=$idwo");
